...
func (t *taskAPI) GetTaskListByCategory(c *gin.Context) {
    ...
    if err != nil {
        if err == repository.ErrNoTasksFound {
            // Menggunakan error khusus untuk kasus tidak ditemukan
            c.JSON(http.StatusNotFound, model.ErrorResponse{Error: "No tasks found for this category"})
            return
        }
        // Untuk error lainnya, kita kembalikan status InternalServerError
        c.JSON(http.StatusInternalServerError, model.ErrorResponse{Error: err.Error()})
        return
    }
